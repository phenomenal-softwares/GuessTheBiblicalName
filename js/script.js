...
let highestScoreTab = document.getElementById('highestScoreTab');
...
highestScoreTab.style.backgroundColor = "gold";
...
const time = new Date().getHours();
  ...
  if (time < 12) {
    greeting = "Good Morning, " + savedUsername + "<br>" + "Have a wonderful day!";
    grtIcon.innerHTML = '<i class="fa-solid fa-sun"></i>'
  }
  else if (time < 16) {
    greeting = "Good Afternoon, " + savedUsername + "<br>" + "How is your day going?";
  }
  else if (time < 20) {
    greeting = "Good Evening, " + savedUsername + "<br>" + "How was your day?";
  }
    else {
    greeting = "Good Night💤 " + savedUsername + "<br>" + "Sleep well!";
...
function randomWord() {
 //progress bar increases by 6.6667%
width = width + 6.6667;
progressBar.style.width = width + '%';
...
gradeColour();
let percentScore = score / 15 * 100;
if (wholeScore.includes(percentScore)) {
   aggScore = percentScore;
 } else {
   aggScore = Number(percentScore.toFixed(1));
 }
if (aggScore > newScore) {
  localStorage.setItem("newScore", aggScore);
} 
   setTimeout(function() {
    modal.style.display = "block";
  document.querySelector("body .wrapper").style.filter = "blur(5px)";
 scoreBorder.innerHTML = aggScore;
